...
static void stackfs_ll_open(fuse_req_t req, fuse_ino_t ino,
  ...
  // : %llu fd : %d", 		lo_name(req, ino), get_higher_fuse_inode_no(req, ino),
  //get_lower_fuse_inode_no(req, ino), fd); StackFS_trace("Open name : %s and
  // inode : %llu", lo_name(req, ino), get_lower_fuse_inode_no(req, ino));
...
static void stackfs_ll_read(fuse_req_t req, fuse_ino_t ino, size_t size,
  ...
  if (USE_SPLICE) {
    ...
    // off : %lu, size : %zu", 			lo_name(req, ino), get_lower_fuse_inode_no(req,
...
static void stackfs_ll_forget(fuse_req_t req, fuse_ino_t ino,
                              uint64_t nlookup) {
...
static int stackfs_process_arg(void *data, const char *arg, int key,
