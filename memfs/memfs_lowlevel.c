...
static const struct fuse_opt memfs_opts[] = {
    MEMFS_OPT("--filename=%s", filename), FUSE_OPT_END};
...
static void mem_ll_init(void *userdata, struct fuse_conn_info *conn) {
  /*
...
static void mem_ll_read(fuse_req_t req, fuse_ino_t ino, size_t size, off_t off,
                        struct fuse_file_info *fi) {
  (void)fi;
  char *buf;
...
int main(int argc, char *argv[]) {
  ...
  se = fuse_session_new(&args, &mem_ll_oper, sizeof(mem_ll_oper), NULL);
  if (se == NULL)
    goto err_out1;
