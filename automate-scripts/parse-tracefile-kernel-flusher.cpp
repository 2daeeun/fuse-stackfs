...
int main(int argc, char **argv) {
  ...
  start:
    while (!input.eof()) {
      if (STRING.find("writeback_pages_before_written:") !=
          string::npos) { /* Flusher Start*/
        iteration++;      /* Flusher Iteration */
                          //				cout<<"Iteration count :
                          //"<<iteration<<"\n";
        getline(input, STRING);
        while (true) {
          /* kworker/u64:3-379   [001] ....  3050.958450:
           * writeback_single_inode: bdi 0:40: ino=11
           * state=I_DIRTY_SYNC|I_DIRTY_PAGES|I_SYNC dirtied_when=4297716349
           * age=0 index=0 to_write=13312 wrote=860 */
          if (STRING.find("writeback_single_inode:") != string::npos) {
            ...
            /* kworker/u64:3-379   [001] ....  3050.958467:
             * writeback_pages_written: 0 */
            stringstream ss;
            ss << STRING;
            string temp;
            int val;
