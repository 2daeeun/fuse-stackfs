...
void get_inode_time(string bigger_string, int max_words, int time_stmp_loc,
                    int inode_loc, unsigned long long *inode,
                    unsigned long long *time_stmp) {
  stringstream ss;
  string temp;
...
void print_map(unordered_map<unsigned long long,
                             vector<vector<unsigned long long>>> &Map1) {
  unordered_map<unsigned long long,
                vector<vector<unsigned long long>>>::iterator iter;
...
int main(int argc, char **argv) {
   ...
   * */
  unordered_map<unsigned long long, vector<vector<unsigned long long>>>
      Map1; /* used for parsing trace.out */
   ...
   * */
  unordered_map<unsigned long long, vector<vector<unsigned long long>>>
      Map2; /* used for parsing trace_stackfs.log */
  ...
  if (input) {
    ...
    while (!input.eof()) {
      ...
      if (STRING.find("vfs_read_start:") != string::npos) {
        /* filebench-14516 [002] ....  4848.804447: vfs_read_start: VFS read
         * start on inode : 39723088 */
        ...
        /* filebench-14516 [002] ....  4848.804459: fuse_read_iter_start: Fuse
         * Read Iter start on inode : 39723088 */
        unsigned long long inode, timestmp;
        ...
        /* filebench-11372 [003] ....   561.870696: fuse_read_request_create:
         * Fuse Read Request created on inode : 31419152 */
        unsigned long long inode, timestmp;

        get_inode_time(STRING, 13, 3, 12, &inode, &timestmp);
        (Map1[inode][2]).push_back(timestmp);
      } else if (STRING.find("fuse_read_iter_end:") != string::npos) {
        /* filebench-14516 [003] ....  4848.813384: fuse_read_iter_end: Fuse
         * Read Iter end on inode : 39723088 */
        unsigned long long inode, timestmp;

        get_inode_time(STRING, 13, 3, 12, &inode, &timestmp);
        (Map1[inode][3]).push_back(timestmp);
      } else if (STRING.find("vfs_read_end:") != string::npos) {
        /* filebench-14516 [003] ....  4848.813386: vfs_read_end: VFS read end
         * on inode : 39723088 */
        unsigned long long inode, timestmp;
  ...
  if (input) {
    ...
    while (!input.eof()) {
      if (STRING.find("StackFS Read start") != string::npos) {
        /* Time : 1479518043328373 Pid : 14506 Tid : 14506 StackFS Read start on
         * inode : 39723088 */
        stringstream ss;
        string temp;
        ...
        /* Time : 1479518043337181 Pid : 14506 Tid : 14506 StackFS Read end on
         * inode : 39723088 */
        stringstream ss;
        string temp;
        unsigned long long int inode, timestmp;
  ...
  unordered_map<unsigned long long,
                vector<vector<unsigned long long>>>::iterator iter1;
  ...
  for (iter1 = Map1.begin(); iter1 != Map1.end(); iter1++) {
    ...
    for (int i = 1; i < V1.size(); i++) {
      ...
      diff2 =
          (V3[i] -
           V2[i]); /* fuse_read_request_create - fuse_read_iter_strt (msecs) */
      diff4 =
          (V5[i] - V4[i]); /* stackFS_read_end - stackFS_read_strt (msecs) */
      tempdiff1 = (V6[i] - V3[i]);
  ...
  for (iter1 = Map3.begin(); iter1 != Map3.end(); iter1++) {
    ...
    output << bar1 << " " << bar2 << " " << bar3 << " " << bar4 << " " << bar5
           << " " << bar6 << "\n";
  }
  output.close();
  return 0;
