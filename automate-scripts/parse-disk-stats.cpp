 ...
 * major  minor  name  Reads        Reads    Sectors  Time Spent     Writes
 * Writes	   Sectors     Time Spent       I/O's      Time Spent weighted
 * time spent completed    merged   Read     Reading(ms)   Completed	 merged
 * Written 	Writing(ms)   Currently   doing I/O's(ms)	I/O's (ms) 8 16
 * sdb    1048          0      8372      164349        216518     1173861
 * 136268152    52716395 	145 	   464559 	      52899933
 *
 ...
 *  Reads_merged = curr_reads_merged - prev_reads_merged (usefull incase of
 ...
 *  writes_merged = curr_writes_merged - prev_writes_merged (usefull incase of
 * sequential writes/reads)
 *
 * */

void print_usage() { cout << "./parse-disk-stats <Folder containing files>\n"; }
...
int main(int argc, char *argv[]) {
  ...
  if (input) {
    ...
    bool first = false;
    string junk;
    unsigned long prev_rds_comp, prev_rds_time, prev_rds_merged, prev_wrs_comp,
        prev_wrs_time, prev_wrs_merged;
    unsigned long curr_rds_comp, curr_rds_time, curr_rds_merged, curr_wrs_comp,
        curr_wrs_time, curr_wrs_merged;
    unsigned long reads_merged, writes_merged;
    double reads_diff, reads_time_diff, read_throughput, writes_diff,
        writes_time_diff, write_throughput;
