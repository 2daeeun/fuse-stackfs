...
int main(int argc, char *argv[]) {
  ...
  if (input) {
    ...
    while (!input.eof()) {
      if (STRING.find("Run took") != string::npos) {
        ...
        /* 21.043: IO Summary:  1026 ops 341.966 ops/s 0/341 rd/wr
         * 341.3mb/s   2.2ms/op */
        //                		outputfilename = outputfolder +
        //                "/throughput.txt"; 		output.open(outputfilename,
        //                std::ofstream::out);
  ...
  /*        float user = 0.0, system = 0.0, iowait = 0.0, idle = 0.0;
     ...
     (iowait/counter) << "\n" << (idle/counter)<<"\n"; output.close(); */
  /*parsing disk stats*/
  /*        float writereqs = 0.0, KBWritten = 0.0, AvgWriteWait = 0.0,
     utilisation = 0.0; counter = 0; outputfilename = outputfolder +
     "/AvgDiskStats.txt"; input.open(disk_stats_file); getline(input, STRING);
          while(!input.eof()) {
  ...
  /*        float memTotal = 0.0, memFree = 0.0, buffers = 0.0, cached = 0.0,
     dirty = 0.0, cntxtSwts = 0.0, memUtil = 0.0; counter = 0; outputfilename =
