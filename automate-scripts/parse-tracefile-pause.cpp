...
int main(int argc, char **argv) {
  ...
  start:
    while (!input.eof()) {
      ...
      loopagain:
        getline(input, STRING);
        while (STRING.find("balance_dirty_pages_debug:") !=
               string::npos) { /* There might be many pauses with in single
                                  iteration of 1MB I/O */
          stringstream ss;
          ss << STRING;
          string temp;
          int val;
          /* filebench-13033 [000] ....  1888.711650: balance_dirty_pages_debug:
           * option : 1 nr_reclaimable : 4887 nr_dirty : 4887 dirty_thresh :
           * 194739 bg_thresh : 97369 bdi_dirty : 1255 bdi_reclaimable : 703
           * bdi_writeback : 552 bdi_bg_thresh : 973 bdi_thresh : 1947
           * bdi_dirty_ratelimit : 25600 pos_ratio : 1100 task_ratelimit : 0
           * min_pause : 0 max_pause : 0 pause : 0 nr_dirtied_pause : 16 */
          i = 0;
          //					cout<<STRING<<"\n";
          while (i < 56) {
            ss >> temp;
        ...
        if (STRING.find("fuse_file_write_iter_end:") != string::npos) {
          ...
          getline(input, STRING);
          goto start;
        } else if (STRING.find("queue_lengths:") !=
                   string::npos) { /* like getxattr */
          /* filebench-18536 [003] .... 89275.429690: queue_lengths: BG Length :
           * 0 Pending Length : 0 Processing Length : 0 */
          //					cout<<"queue lengths line :
          //"<<STRING<<"\n";
          stringstream ss, ss1;
          ss << STRING;
          string temp;
  ...
  /* Dirty pages Pause points (How many dirty pages after which this check is
   * called again) */
