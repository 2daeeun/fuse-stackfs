...
void print_usage() { cout << "./parse-output <Folder containing files>\n"; }
...
string get_req_type(int type) { return Req_Map[type]; }
...
int main(int argc, char *argv[]) {
  ...
  unsigned long long int background[46][34], pending[46][34],
      processing[46][34], user_processing[46][34];
  outputfolder = argv[1];
  //	std::string::size_type sz;
  string kernel_bg, kernel_pending, kernel_processing, user_proc_file,
      filebench_out, cpu_stats_file, mem_stats_file, disk_stats_file;
  vector<string> temp;
  int i = 1;
  ...
  /*generate output file containg only request types and their count in each
   * queue*/
  ...
  for (int i = 1; i <= 45; i++) {
    if (background[i][33] > 0 || pending[i][33] > 0 || processing[i][33] > 0 ||
        user_processing[i][33] > 0) {
      ...
      dist_outputfilename =
          outputfolder + "/" + get_req_type(i) + "_background_distribution.txt";
      system(("rm -rf " + dist_outputfilename).c_str());
      output1.open(dist_outputfilename,
      ...
      dist_outputfilename =
          outputfolder + "/" + get_req_type(i) + "_pending_distribution.txt";
      system(("rm -rf " + dist_outputfilename).c_str());
      output1.open(dist_outputfilename,
                   std::ofstream::out | std::ofstream::app);
      for (int j = 0; j < 34; j++)
      ...
      dist_outputfilename =
          outputfolder + "/" + get_req_type(i) + "_processing_distribution.txt";
      system(("rm -rf " + dist_outputfilename).c_str());
      output1.open(dist_outputfilename,
                   std::ofstream::out | std::ofstream::app);
      for (int j = 0; j < 34; j++)
      ...
      dist_outputfilename = outputfolder + "/" + get_req_type(i) +
                            "_user_processing_distribution.txt";
      system(("rm -rf " + dist_outputfilename).c_str());
      output1.open(dist_outputfilename,
                   std::ofstream::out | std::ofstream::app);
      for (int j = 0; j < 34; j++)
  ...
  /*parsing cpu stats*/
  double tot_cpu, cpu_0, cpu_1, cpu_2, cpu_3, cpu_4, cpu_5, cpu_6,
      cpu_7; /* I know there are 8 cores */
  ...
  /*	float user = 0.0, system = 0.0, iowait = 0.0, idle = 0.0;
     ...
     (iowait/counter) << "\n" << (idle/counter)<<"\n"; output.close();
  */	/*parsing disk stats*/
  /*	float writereqs = 0.0, KBWritten = 0.0, AvgWriteWait = 0.0, utilisation
     = 0.0; counter = 0; outputfilename = outputfolder + "/AvgDiskStats.txt";
  ...
  */	/*parsing memory stats*/
  /*	float memTotal = 0.0, memFree = 0.0, buffers = 0.0, cached = 0.0, dirty
     = 0.0, cntxtSwts = 0.0, memUtil = 0.0; counter = 0; outputfilename =
     outputfolder + "/AvgMemStats.txt"; input.open(mem_stats_file);
          getline(input, STRING);
          while(!input.eof()) {
  ...
  */                        /*Context Switches*/
  /*                        getline(input, STRING);
                          ...
                          if (str_cntxt == 0)
                                  str_cntxt = cntxtSwts;
                          memUtil += (memTotal - memFree - buffers -
     cached)/memTotal;
