 ...
 * guest  guest_nice cpu   74608    2520   24433   1117073      6176   4054 0 0
 * 0      0
 *
...
void print_usage() { cout << "./parse-cpu-stats <Folder containing files>\n"; }
...
int main(int argc, char *argv[]) {
  ...
  if (input) {
    ...
    bool first = false;
    string junk;
    unsigned long prev_idle, prev_iowait, prev_user, prev_nice, prev_system,
        prev_irq, prev_softirq, prev_steal;
    unsigned long curr_idle, curr_iowait, curr_user, curr_nice, curr_system,
        curr_irq, curr_softirq, curr_steal;
    double prev_tot_idle /* (prev_idle + prev_iowait) */,
        prev_tot_nonidle /* (prev_user + prev_nice + prev_system + prev_irq +
                            prev_softirq + prev_steal ) */
        ,
        prev_tot /* (prev_tot_idle + prev_tot_nonidle) */;
    double curr_tot_idle /* (curr_idle + curr_iowait) */,
        curr_tot_nonidle /* (curr_user + curr_nice + curr_system + curr_irq +
                            curr_softirq + curr_steal ) */
        ,
        curr_tot /* (curr_tot_idle + curr_tot_nonidle) */;
    double tot_diff /* (curr_tot - prev_tot ) */,
    ...
    while (!input.eof()) {
      ...
      if (started && STRING.find("cpu ") != string::npos) {
        ...
        if (!first) {
          ...
          /* add up the jiffies */
          user_jiffies.push_back((curr_user - prev_user) +
                                 (curr_nice - prev_nice));
  ...
  for (int i = 0; i < cpu_util.size(); i++)
    output << cpu_util[i] << "	" << cpu_user[i] << "	"
           << cpu_nice[i] << "	" << cpu_system[i] << "\n";
  //	for (int i = 0; i < cpu_util.size(); i++) {
  //		cout<<"CPU util : "<<cpu_util[i] <<" user : "<<cpu_user[i]<<"
  //nice : "<<cpu_nice[i]<<" system : "<<cpu_system[i]<<"\n"; 		cout<<"CPU idle :
  //"<<cpu_tot_idle[i]<<" idle : "<<cpu_idle[i]<<" iowait :
  //"<<cpu_iowait[i]<<"\n";
  //	}
  output.close();
  ...
  for (int i = 0; i < cpu_tot_idle.size(); i++)
    output << cpu_tot_idle[i] << "	" << cpu_idle[i] << "	"
           << cpu_iowait[i] << "\n";
  output.close();
