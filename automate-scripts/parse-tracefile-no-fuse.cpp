...
int main(int argc, char **argv) {
  ...
  unordered_map<unsigned long, vector<double>>
      Map; /* inode -> list of read diffs for every iteration */
  ...
  if (input) {
    ...
    while (!input.eof()) {
      if (STRING.find("filemap_generic_read_iter_difference:") !=
          string::npos) {
        i = 0;
        stringstream ss;
        ss << STRING;
...
/* filebench-11808 [002] ....  1935.487393:
 * filemap_generic_read_iter_difference: Generic Read on inode : 5767184 time
 * diff : 682 */
