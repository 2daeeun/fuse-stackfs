...
/*
 * StackFS_ll-18865 [002] .... 25281.345892: bg_queue_difference: Bg Queue on
 * inode : 12095568 time diff : 95 StackFS_ll-18865 [002] .... 25281.345900:
 * pending_queue_difference: Pending Queue on inode : 12095568 time diff : 49308
 * StackFS_ll-18865 [002] .... 25281.345900: processing_queue_difference:
 * Processing Queue on inode : 12095568 time diff : 6746632
 * <...>-18873 [000] .... 25281.345940: fuse_read_difference: Fuse Read on inode
 * : 12095568 time diff : 6851970 StackFS_ll-31362 [005] .... 216739.246437:
 * queue_lengths: BG Length : 0 Pending Length : 0 Processing Length : 13
 * */
...
int main(int argc, char **argv) {
  ...
  unordered_map<unsigned long, vector<vector<double>>>
      Map; /* inode -> list of read/bg/pending/processing diffs for every
              iteration */
  /*
  ...
  if (input) {
    ...
    while (!input.eof()) {
      if (STRING.find("bg_queue_difference:") != string::npos) {
        ...
        // cout<<"Read difference inode : "<<inode<<" milli secs :
        // "<<milli_secs<<"\n";
        if (Map.count(inode)) {
  ...
  output1.open(outputfilename1.c_str(),
               std::ofstream::out | std::ofstream::app);
  output2.open(outputfilename2.c_str(),
               std::ofstream::out | std::ofstream::app);
  output3.open(outputfilename3.c_str(),
               std::ofstream::out | std::ofstream::app);
  output4.open(outputfilename4.c_str(),
               std::ofstream::out | std::ofstream::app);
  output5.open(outputfilename5.c_str(),
               std::ofstream::out | std::ofstream::app);
  output6.open(outputfilename6.c_str(),
               std::ofstream::out | std::ofstream::app);
  output7.open(outputfilename7.c_str(),
               std::ofstream::out | std::ofstream::app);
